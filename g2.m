...
arr_opt_enegery_used = zeros(length(upper_N_ar),length(v_ar));
arr_static_enegery_used = zeros(length(upper_N_ar),length(v_ar));
arr_task_iter_cnt = 1;
...
%   
  ...
  C =  randi([10 35],1,m); % server link capacity (# of tasks)
  C(end)=inf;
...
arr_opt_enegery_used(uu,vv) = opt_enegery_used;
arr_static_enegery_used(uu,vv)= static_enegery_used;
